...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  // retrieve absolute path of the file.
  char file_abs[PATH_MAX];
  if (realpath(test_file.c_str(), file_abs) == nullptr) {
    // return false if unable to access the file.
    return false;
  }
  string file_path = file_abs;

  // retrieve absolute path of the directory.
  char direct_abs[PATH_MAX];
  if (realpath(root_dir.c_str(), direct_abs) == nullptr) {
    // return false if unable to access the directory.
    return false;
  }
  string direct_path = direct_abs;

  // ensure the directory path ends with "/".
  direct_path.append("/");

  // check if the directory path exists within the file path.
  size_t dir_pos = file_path.rfind(direct_path);
  if (dir_pos == string::npos) {
    // return false if the directory is not found in the file path.
    return false;
  }

  // directory found within the file path, return true.
  return true;
}
