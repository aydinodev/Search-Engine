...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  // if doc_name already exists, returns doc id.
  if (HashTable_Find(table->name_to_id, FNVHash64((unsigned char*) doc_name,
                    strlen(doc_name)), &old_kv)) {
    return *((DocID_t*) old_kv.value);
  }

  // if doc_name doesn't exist, add to the tables.
  // allocates memory for copy of doc_name string copy.
  doc_copy = (char*) malloc(sizeof(char) * (strlen(doc_name) + 1));

  Verify333(doc_copy != NULL);
  snprintf(doc_copy, sizeof(char) * (strlen(doc_name) + 1), "%s", doc_name);
  ...
  *doc_id = table->max_id;
  ...
  // update kv, key is hashed version of doc_name.
  // kv value is pointer to the doc_id.
  kv.key = FNVHash64((unsigned char*) doc_copy, strlen(doc_copy));
  kv.value = doc_id;
  // inserts name into table.
...
DocID_t DocTable_GetDocID(DocTable* table, char* doc_name) {
  ...
  // if doc exists in table, returns doc_id.
  if (HashTable_Find(table->name_to_id, key, &kv)) {
    return *((DocID_t*) kv.value);
  }
  // if doc doesn't exist, returns invalid doc id.
...
char* DocTable_GetDocName(DocTable* table, DocID_t doc_id) {
  ...
  // if doc_id exists in table, returns string.
  if (HashTable_Find(table->id_to_name, doc_id, &kv)) {
    return (char*) kv.value;
  }
  // if doc_id doesn't exist in table, return NULL.
